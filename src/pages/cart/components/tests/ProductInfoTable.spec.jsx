...
it('장바구니에 포함된 아이템들의 이름, 수량, 합계가 제대로 노출된다', async () => {
  ...
  // NOTE: within - 이미 조회한 요소를 특정하고 싶은 경우에 사용
  // 첫번째 요소 검증
  expect(
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  expect(screen.getByText('Awesome Concrete Shirt')).toBeInTheDocument(); // 클릭 전에 잘 렌더링 되었는지 먼저 확인

  await user.click(deleteButton);

  // queryBy~: 요소의 존재 유무 판단. 요소가 존재하기 않아도 에러 발생하지 않음.
